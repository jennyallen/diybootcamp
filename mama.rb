...
post '/scheduler/yournewschedule' do 
	...
	daysOfWeek.each do |day|
		starttime = timeToNum(params[day+"-starttime"])
	 	...
	 	weekavail.day = DayAvailability.new(day, createDayArray(starttime, endtime), endtime-starttime)
	end

	#hours Per Week that you are willing to work
	hoursperpeek = params['hoursPerWeek']

	weeksched = (startdate, weekAvail.weekArray, hoursperweek)


	#create a schedule of weekly availability
	weeklyavail = weeksched.createSchedule

	#create total schedule 
	totalsched = TotalSchedule.new(58, hoursperweek)
	...
	totalsched.createScheduleArr(h)

	puts totalsched.courseArray
